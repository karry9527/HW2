...
void mesh::ComputeTangentBasis()
{
	...
	{
		cout << "v :" << vList[faceList[0][0].v].ptr[j] << "  " << vList[faceList[0][1].v].ptr[j] << "  " << vList[faceList[0][2].v].ptr[j] << endl; 
	}
	for (size_t j=0; j < 2; j++)
	{
		cout << "t :" << tList[faceList[0][0].t].ptr[j] << "  " << tList[faceList[0][1].t].ptr[j] << "  " << tList[faceList[0][2].t].ptr[j] << endl; 
	}
	cout << "f:" << faceList[0][1].v << endl;
	
	float v[3] = {0, 0, 0};
	Edge1.push_back(Vec3(v));
	Edge2.push_back(Vec3(v));
	Edgeuv1.push_back(Vec3(v));
	Edgeuv2.push_back(Vec3(v));
	for (size_t i=0;i < fTotal;i++)
		{
			
			for (size_t j=0; j < 3; j++)
			{
				Edge1[0].ptr[j] = vList[faceList[i][1].v].ptr[j] - vList[faceList[i][0].v].ptr[j];
				Edge2[0].ptr[j] = vList[faceList[i][2].v].ptr[j] - vList[faceList[i][0].v].ptr[j];
				if(j < 2)
				{
					Edgeuv1[0].ptr[j] = tList[faceList[i][1].t].ptr[j] - tList[faceList[i][0].t].ptr[j];
					Edgeuv2[0].ptr[j] = tList[faceList[i][2].t].ptr[j] - tList[faceList[i][0].t].ptr[j];
				}
			}
			float cp = Edgeuv1[0].ptr[0] * Edgeuv2[0].ptr[1] - Edgeuv2[0].ptr[0] * Edgeuv1[0].ptr[1];
			if(cp != 0.0f)
			{
				float mul = 1.0f/cp, tvec[3], bvec[3];
				for(size_t j=0; j < 3; j++)
				{
					tvec[j] = (Edge1[0].ptr[j] * Edgeuv2[0].ptr[1] + Edge2[0].ptr[j] * -Edgeuv1[0].ptr[1]) * mul;
					bvec[j] = (Edge1[0].ptr[j] * -Edgeuv2[0].ptr[0] + Edge2[0].ptr[j] * Edgeuv1[0].ptr[0]) * mul;

				}
				
				tangentList.push_back(Vec3(tvec));
				bitangentList.push_back(Vec3(bvec));
			}
			
				//printf("%f %f %f", tangentList[i].ptr[0], tangentList[i].ptr[1], tangentList[i].ptr[2]);
			
		}
	
	cout << "tan :" << tangentList[0].ptr[0] << "  " << tangentList[0].ptr[1] << "  " << tangentList[0].ptr[2] << endl; 
	
	cout << "bitan :" << bitangentList[0].ptr[0] << "  " << bitangentList[0].ptr[1] << "  " << bitangentList[0].ptr[2] << endl; 


	tanTotal = tangentList.size();
	bitanTotal = bitangentList.size();
	printf("tan: %d, bitan: %d\n", tanTotal, bitanTotal);
	cout << "ptr test: "<< tangentList[0].ptr << endl;
}
/*void mesh::ComputeTangentBasis()
{
	for (size_t j=0; j < 3; j++)
	...
	for (size_t i=0;i < fTotal;i++)
		{
			...
			for (size_t j=0; j < 3; j++)
			{
				Edge1[0].ptr[j] = vList[faceList[i][1].v].ptr[j] - vList[faceList[i][0].v].ptr[j];
				Edge2[0].ptr[j] = vList[faceList[i][2].v].ptr[j] - vList[faceList[i][0].v].ptr[j];
				Edge1[1].ptr[j] = vList[faceList[i][0].v].ptr[j] - vList[faceList[i][1].v].ptr[j];
				Edge2[1].ptr[j] = vList[faceList[i][2].v].ptr[j] - vList[faceList[i][1].v].ptr[j];
				Edge1[2].ptr[j] = vList[faceList[i][1].v].ptr[j] - vList[faceList[i][2].v].ptr[j];
				Edge2[2].ptr[j] = vList[faceList[i][0].v].ptr[j] - vList[faceList[i][2].v].ptr[j];
				if(j < 2)
				{
					Edgeuv1[0].ptr[j] = tList[faceList[i][1].t].ptr[j] - tList[faceList[i][0].t].ptr[j];
					Edgeuv2[0].ptr[j] = tList[faceList[i][2].t].ptr[j] - tList[faceList[i][0].t].ptr[j];
					Edgeuv1[1].ptr[j] = tList[faceList[i][0].t].ptr[j] - tList[faceList[i][1].t].ptr[j];
					Edgeuv2[1].ptr[j] = tList[faceList[i][2].t].ptr[j] - tList[faceList[i][1].t].ptr[j];
					Edgeuv1[2].ptr[j] = tList[faceList[i][1].t].ptr[j] - tList[faceList[i][2].t].ptr[j];
					Edgeuv2[2].ptr[j] = tList[faceList[i][0].t].ptr[j] - tList[faceList[i][2].t].ptr[j];
				}
			}
			float cp1 = Edgeuv1[0].ptr[0] * Edgeuv2[0].ptr[1] - Edgeuv2[0].ptr[0] * Edgeuv1[0].ptr[1];
			float cp2 = Edgeuv1[1].ptr[0] * Edgeuv2[1].ptr[1] - Edgeuv2[1].ptr[0] * Edgeuv1[1].ptr[1];
			float cp3 = Edgeuv1[2].ptr[0] * Edgeuv2[2].ptr[1] - Edgeuv2[2].ptr[0] * Edgeuv1[2].ptr[1];
			if(cp1 != 0.0f)
			{
				float mul = 1.0f/cp1, tvec[3], bvec[3];
				for(size_t j=0; j < 3; j++)
				{
					tvec[j] = (Edge1[0].ptr[j] * Edgeuv2[0].ptr[1] + Edge2[0].ptr[j] * Edgeuv1[0].ptr[1]) * mul;
					bvec[j] = (Edge1[0].ptr[j] * -Edgeuv2[0].ptr[0] + Edge2[0].ptr[j] * Edgeuv1[0].ptr[0]) * mul;
			...
			if(cp2 != 0.0f)
			{
				float mul = 1.0f/cp2, tvec[3], bvec[3];
				for(size_t j=0; j < 3; j++)
				{
					tvec[j] = (Edge1[1].ptr[j] * Edgeuv2[1].ptr[1] + Edge2[1].ptr[j] * Edgeuv1[1].ptr[1]) * mul;
					bvec[j] = (Edge1[1].ptr[j] * -Edgeuv2[1].ptr[0] + Edge2[1].ptr[j] * Edgeuv1[1].ptr[0]) * mul;
			...
			if(cp3 != 0.0f)
			{
				float mul = 1.0f/cp3, tvec[3], bvec[3];
				for(size_t j=0; j < 3; j++)
				{
					tvec[j] = (Edge1[2].ptr[j] * Edgeuv2[2].ptr[1] + Edge2[2].ptr[j] * Edgeuv1[2].ptr[1]) * mul;
					bvec[j] = (Edge1[2].ptr[j] * -Edgeuv2[2].ptr[0] + Edge2[2].ptr[j] * Edgeuv1[2].ptr[0]) * mul;
