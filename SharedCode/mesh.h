...
class mesh  
{
	...
	class Vec3		// vList, nList, tList 的 structure and tangent, bitangent 
	{
	public:
		float ptr[3];
		Vec3 (float *v) 
...
public:
	...
	std::vector<Vec3>			tangentList;		// tangent List
	std::vector<Vec3>			bitangentList;	// bitangent List
	std::vector<Vec3>			Edge1;			
	std::vector<Vec3>			Edge2;	
	std::vector<Vec3>			Edgeuv1;
	std::vector<Vec3>			Edgeuv2;
	std::vector<FACE>			faceList;		// Face List



	size_t	vTotal, tTotal, nTotal, fTotal, tanTotal, bitanTotal; // number of total vertice, faces, texture coord., normal vecters, and faces
